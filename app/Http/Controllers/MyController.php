...
class MyController extends Controller
{
    ...
    //         if ($request->password == $user->pwd) {
    //             # code...
    //             Session::put('loginId', $user->idutilisateur);
    //             $data = [
    //                 'sport' => Sport::all()
    //             ];
    //             return view('pages/gestion', compact('data'));
    //         } else {
    //             return back()->with('fail', 'Mot de passe incorrect');
    //         }
    //     } else {
    ...
    public function getConsommation(Request $request){
        ...
        $taux_creuse = (double)$conso->taux_pers_h_creuse;
        $nbPers = (double)$conso->nb_pers;
        $consoFix = (double)$conso->consommation_fix;
        $puissMoy = (double)$conso->puissance_moy;
        $tauxEleve = $taux;
        $Consommation = [];
        $ConsommationFix = 0;
        $ConsommationEleve = 0;
